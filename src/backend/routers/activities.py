...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level
        - If not provided: Returns all activities regardless of difficulty
        - If 'Beginner', 'Intermediate', or 'Advanced': Returns only activities with that specific difficulty
        - If 'All': Returns only activities with NO difficulty field (suitable for all levels)
    """
    ...
    if difficulty:
        if difficulty == "All":
            # "All" means activities with NO difficulty specified (for all levels)
